...
const AUDIO_VARIANTS = { original: "origineel", denoised: "ruisonderdrukt", isolated: "geïsoleerde doelstem", extracted: "legacy-extractie" };
const state = {
...
function renderAnalysisDetail(item) {
  ...
  const variants = [
    ...
    ["isolated", elements.analysisIsolatedAudio, elements.isolatedUnavailable],
  ];
  ...
  metadataList(elements.pipelineMetadata, [["Bron", item.source || "pipeline"], ["Satelliet", item.satellite_name || item.satellite], ["STT-engine", item.stt_entity_id || item.stt_entity], ["Herkenning", timing.recognition_ms != null ? `${timing.recognition_ms} ms` : item.recognition_ms != null ? `${item.recognition_ms} ms` : "—"], ["Ruisonderdrukking", timing.denoise_ms != null ? `${timing.denoise_ms} ms` : "—"], ["Isolatie", timing.isolation_ms != null ? `${timing.isolation_ms} ms` : "—"], ["Extra audiobewerking", timing.audio_processing_ms != null ? `${timing.audio_processing_ms} ms` : timing.total_ms != null && !timing.stt_ms ? `${timing.total_ms} ms` : "—"], ["STT-tijd", timing.stt_ms != null ? `${timing.stt_ms} ms` : "—"], ["Totale pipeline", timing.total_ms != null ? `${timing.total_ms} ms` : "—"], ["Modelstatus", stageText || "niet geladen"], ["Verwerking", item.processing_status || "niet gestart"], ["Fallbackreden", item.processing_fallback_reason], ["Duur behouden", quality.duration_preserved == null ? null : quality.duration_preserved ? "Ja" : "Nee"], ["RMS origineel", quality.original_rms], ["RMS ruisonderdrukt", quality.denoised_rms], ["RMS doelstem", quality.isolated_rms], ["Clipping", quality.clipping_ratio == null ? null : `${(quality.clipping_ratio * 100).toFixed(2)}%`], ["Stilte", quality.silence_ratio == null ? null : `${(quality.silence_ratio * 100).toFixed(1)}%`], ["Denoised speaker-score", quality.denoised_target_score == null ? null : formatScore(quality.denoised_target_score)], ["Geïsoleerde speaker-score", quality.isolated_target_score == null ? null : formatScore(quality.isolated_target_score)], ["Isolatiemarge", quality.isolated_margin == null ? null : formatScore(quality.isolated_margin)], ["Gebruikte audio", AUDIO_VARIANTS[item.audio_variant] || item.audio_variant], ["Geblokkeerd", item.blocked ? "Ja" : "Nee"], ["Conversation doorgestuurd", item.conversation_forwarded == null ? "—" : item.conversation_forwarded ? "Ja" : "Nee"]]);
...
async function extractAudio() { const item = state.analysis.detail; const speakerId = elements.extractSpeaker.value; if (!item || !speakerId) { showToast("Kies eerst een doelprofiel."); return; } elements.extractAudio.disabled = true; setText(elements.extractAudio, "Isolatie starten…"); try { await request(`api/analysis/${encodeURIComponent(getId(item))}/process`, { method: "POST", body: JSON.stringify({ speaker_id: speakerId }) }); showToast("Doelstemisolatie gestart"); await pollProcessing(getId(item)); } catch (error) { showToast(error.message); } finally { elements.extractAudio.disabled = false; setText(elements.extractAudio, "Doelstem isoleren"); } }
async function pollProcessing(id) { for (let attempt = 0; attempt < 225; attempt += 1) { await new Promise((resolve) => setTimeout(resolve, 800)); const detail = await request(`api/analysis/${encodeURIComponent(id)}`); state.analysis.detail = detail; renderAnalysisDetail(detail); if (!["queued", "running"].includes(detail.processing_status)) { if (detail.processing_status === "complete" && detail.isolated_available) showToast("Doelstemisolatie afgerond"); else showToast(detail.processing_fallback_reason || "Isolatie gebruikt de beste beschikbare fallback"); return; } } showToast("Verwerking loopt nog; open dit detail later opnieuw."); }
...
function closeAnalysis() { if (elements.analysisDialog.open) elements.analysisDialog.close(); [elements.analysisOriginalAudio, elements.analysisDenoisedAudio, elements.analysisIsolatedAudio].forEach((player) => { player.pause(); player.removeAttribute("src"); }); state.analysis.detail = null; state.analysis.waveform = null; }
