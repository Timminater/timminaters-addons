...
const state = {
  ...
  analysis: { items: [], total: 0, offset: 0, limit: 24, selected: new Set(), detail: null, audioUrl: null, duration: 0, trim: { start: 0, end: 0 }, waveform: null, selectionPreview: false, previewFrame: null },
...
function routeFromHash() { return ["profiles", "analysis", "calibration", "settings"].includes(location.hash.slice(1)) ? location.hash.slice(1) : "profiles"; }
...
async function loadSettings() {
  try {
    const policy = await request("api/pipeline-policy");
    state.overview = { ...state.overview, ...policy };
    renderPolicy(policy);
...
async function savePolicy() {
  ...
  try {
    const policy = await request("api/pipeline-policy", { method: "PATCH", body: JSON.stringify({ unknown_speaker_policy: elements.unknownSpeakerPolicy.value, extraction_mode: elements.extractionMode.value, min_margin: minMargin, audio_processing_backend: elements.audioProcessingBackend.value, retention_days: retentionDays, max_storage_bytes: maxStorageBytes }) });
...
async function loadPersons() { elements.speakerPerson.replaceChildren(new Option(tr("Personen laden…"), "")); try { state.persons = await request("api/home-assistant-persons"); elements.speakerPerson.replaceChildren(new Option(tr("Niet gekoppeld"), ""), ...state.persons.map((person) => new Option(person.name, person.entity_id))); } catch (error) { elements.speakerPerson.replaceChildren(new Option(tr("Personen niet bereikbaar"), "")); showToast(error.message); } }
