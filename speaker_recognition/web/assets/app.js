...
function populateSpeakerOptions() {
  ...
  elements.analysisSpeaker.replaceChildren(new Option(tr("Alle sprekers"), ""), ...state.speakers.map((speaker) => new Option(speaker.name, getId(speaker))));
...
function renderAnalysis() {
  ...
  state.analysis.items.forEach((item) => {
    ...
    const title = appendText(card, "strong", detectedNames.length > 1 ? detectedNames.join(" + ") : item.speaker_name || item.speaker?.name || item.person_name || "Onbekende stem", "analysis-speaker");
...
function renderCalibration(data) {
  ...
  const enough = genuine >= 3 && impostor >= 3; elements.calibrationWarning.hidden = enough; if (!enough) setText(elements.calibrationWarning, `Nog ${Math.max(0, 3 - genuine)} metingen van dezelfde spreker en ${Math.max(0, 3 - impostor)} metingen van andere sprekers nodig voordat je een advies kunt toepassen.`);
  setText(elements.recommendedTitle, enough && preview.ready !== false ? "Veilige aanbeveling" : "Nog onvoldoende gegevens"); setText(elements.recommendedThreshold, recommendation.threshold != null ? formatScore(recommendation.threshold) : "—"); setText(elements.recommendedMargin, recommendation.margin != null ? formatScore(recommendation.margin) : "—"); setText(elements.falseAccepts, recommendation.false_accepts ?? recommendation.false_accept_count ?? "—"); setText(elements.falseRejects, recommendation.false_rejects ?? recommendation.false_reject_count ?? "—"); setText(elements.calibrationDetail, recommendation.summary || "Een verkeerde toewijzing telt vier keer zo zwaar als een gemiste herkenning."); elements.applyCalibration.disabled = !enough || preview.ready === false || recommendation.threshold == null; drawCalibration(preview.genuine_scores || preview.genuine || [], preview.impostor_scores || []);
}
