...
const AUDIO_VARIANTS = { original: "origineel", extracted: "geëxtraheerd" };
const state = {
...
const elements = Object.fromEntries([
  ...
  "analysis-dialog analysis-dialog-title analysis-detail-loading analysis-detail analysis-original-audio analysis-extracted-audio extracted-unavailable analysis-waveform trim-start trim-end waveform-range recognition-metadata pipeline-metadata extract-speaker extract-audio download-original promote-audio delete-analysis",
...
function renderAnalysisDetail(item) {
  ...
  const available = Boolean(item.extracted_available ?? item.has_extracted_audio ?? item.extracted_audio_url);
  elements.analysisExtractedAudio.hidden = !available; elements.extractedUnavailable.hidden = available;
  if (available) { elements.analysisExtractedAudio.src = item.extracted_audio_url || audioEndpoint(id, "extracted"); elements.analysisExtractedAudio.load(); }
  ...
  metadataList(elements.pipelineMetadata, [["Bron", item.source || "pipeline"], ["Satelliet", item.satellite_name || item.satellite], ["STT", item.stt_entity_id || item.stt_entity], ["Herkenning", timing.recognition_ms != null ? `${timing.recognition_ms} ms` : item.recognition_ms != null ? `${item.recognition_ms} ms` : "—"], ["Extractie", timing.extraction_ms != null ? `${timing.extraction_ms} ms` : item.extraction_ms != null ? `${item.extraction_ms} ms` : "—"], ["STT-duur", timing.stt_ms != null ? `${timing.stt_ms} ms` : item.stt_ms != null ? `${item.stt_ms} ms` : "—"], ["Totaal", timing.total_ms != null ? `${timing.total_ms} ms` : item.total_ms != null ? `${item.total_ms} ms` : "—"], ["Extractiemodus", item.extraction_mode], ["Extractiestatus", item.extraction_status], ["Gebruikte audio", AUDIO_VARIANTS[item.audio_variant] || item.audio_variant], ["Fallback naar origineel", item.fallback == null ? "—" : item.fallback ? "Ja" : "Nee"], ["Geblokkeerd", item.blocked ? "Ja" : "Nee"], ["Conversation doorgestuurd", item.conversation_forwarded == null ? "—" : item.conversation_forwarded ? "Ja" : "Nee"], ["Conversation-reden", item.conversation_reason]]);
  elements.extractSpeaker.replaceChildren(new Option("Kies een profiel", ""), ...state.speakers.map((speaker) => new Option(speaker.name, getId(speaker))));
...
async function extractAudio() { const item = state.analysis.detail; const speakerId = elements.extractSpeaker.value; if (!item || !speakerId) { showToast("Kies eerst een doelprofiel."); return; } elements.extractAudio.disabled = true; setText(elements.extractAudio, "Extraheren…"); try { await request(`api/analysis/${encodeURIComponent(getId(item))}/extract`, { method: "POST", body: JSON.stringify({ speaker_id: speakerId }) }); showToast("Extractie gestart. Vernieuw het detail na afronden."); } catch (error) { showToast(error.message); } finally { elements.extractAudio.disabled = false; setText(elements.extractAudio, "Nu extraheren"); } }
...
function closeAnalysis() { if (elements.analysisDialog.open) elements.analysisDialog.close(); elements.analysisOriginalAudio.pause(); elements.analysisExtractedAudio.pause(); elements.analysisOriginalAudio.removeAttribute("src"); elements.analysisExtractedAudio.removeAttribute("src"); state.analysis.detail = null; state.analysis.waveform = null; }
