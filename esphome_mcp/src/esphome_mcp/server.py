...
_auth = (
    StaticTokenVerifier(
        tokens={
            _auth_token: {
                "sub": "esphome-mcp-client",
                "client_id": "esphome-mcp-client",
                "scopes": ["read", "write"],
            }
        }
    )
...
mcp = FastMCP(name="ESPHome MCP", instructions=INSTRUCTIONS, auth=_auth)


def _local_files_enabled() -> bool:
    """Return whether MCP tools may access the server's local filesystem."""
    return os.environ.get("ESPHOME_MCP_ALLOW_LOCAL_FILES", "true").lower() in {
        "1",
        "true",
        "yes",
    }
...
def _local_files_disabled_message() -> str:
    return (
        "Error: local file access is disabled on this server. "
        "Pass YAML inline or use a dashboard device name."
    )
...
async def get_device_configuration(device_name: str, output_path: str | None = None) -> str:
    ...
    if output_path is not None:
        if not _local_files_enabled():
            return _local_files_disabled_message()
        try:
